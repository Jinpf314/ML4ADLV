...
alpha = dict()
alpha[1]=vect([1,-1,0,0,0,0])
alpha[2]=vect([0,1,-1,0,0,0])
alpha[3]=vect([0,0,1,-1,0,0])
alpha[4]=vect([0,0,0,1,-1,0])
alpha[5]=vect([0,0,0,0,1,-1])

omega=dict()
omega[6]=vect([1,1,1,1,1,1])
omega[5]=vect([1,1,1,1,1,0])
omega[4]=vect([1,1,1,1,0,0])
omega[3]=vect([1,1,1,0,0,0])
omega[2]=vect([1,1,0,0,0,0])
omega[1]=vect([1,0,0,0,0,0])
...
'''
omega15 = rational_vect([1,0,0,0,0,-1])
omega24 = rational_vect([1,1,0,0,-1,-1])
omega3 = rational_vect([1/2,1/2,1/2,-1/2,-1/2,-1/2])
omega_sigma_set = {omega15,omega24,omega3}
###################################################
######################################################
#####################################################
...
for i in S0key:
    S0 = S0|{s[i]}
W0 = joblib.load('A5_W0')
Phiplus = joblib.load('A5_Phiplus')
S={s[0]}|S0
two_rho = joblib.load('A5_two_rho')
rho = joblib.load('A5_rho')
...
theta = vect([1,0,0,0,0,-1])

y01 = exp([2,3,2,4,3,2,5,4,3,2]) + w_0
y02 = exp([1,3,4,3,5,4,3]) + w_0
y03 = exp([1,2,1,4,5,4]) + w_0
y04 = exp([1,2,1,3,2,1,5]) + w_0
...
Omega0 = {tau1, tau2,tau3,tau4,tau5}
Omega = {Id,tau1, tau2,tau3,tau4,tau5}  
...
reflection_dict[s[4]+s[5]+s[4]] = alpha[4]+alpha[5]

reflection_dict[s[1]+s[2]+s[3]+s[2]+s[1]] = alpha[1]+alpha[2]+alpha[3]
reflection_dict[s[2]+s[3]+s[4]+s[3]+s[2]] = alpha[2]+alpha[3]+alpha[4]
reflection_dict[s[3]+s[4]+s[5]+s[4]+s[3]] = alpha[3]+alpha[4]+alpha[5]

reflection_dict[s[1]+s[2]+s[3]+s[4]+s[3]+s[2]+s[1]] = alpha[1]+alpha[2]+alpha[3]+alpha[4]
reflection_dict[s[2]+s[3]+s[4]+s[5]+s[4]+s[3]+s[2]] = alpha[2]+alpha[3]+alpha[4]+alpha[5]

reflection_dict[s[1]+s[2]+s[3]+s[4]+s[5]+s[4]+s[3]+s[2]+s[1]] = alpha[1]+alpha[2]+alpha[3]+alpha[4]+alpha[5]
...
#joblib.dump(O_length_dict,'A5_O_length_dict')
'''


O_length_dict = joblib.load('A5_O_length_dict')
O_length_dict_tau1 = joblib.load('A5_O_length_dict_tau1')
O_length_dict_tau2 = joblib.load('A5_O_length_dict_tau2')
O_length_dict_tau3 = joblib.load('A5_O_length_dict_tau3')
O_length_dict_tau4 = joblib.load('A5_O_length_dict_tau4')
O_length_dict_tau5 = joblib.load('A5_O_length_dict_tau5')
...
def isminimal(w):
    ...
    if wp in O_length_dict_tau5:
        return(O_length_dict_tau5[wp] == w.len())   
...
'''
Admmu3 = Adm(mu3,tau3)
joblib.dump(Admmu3,'A5_Admmu3')
    
BGmu3 = set()
for i in Admmu3:
    for w in Admmu3[i]:
        BGmu3 = vectunion(BGmu3,{rational_vect(w.Newton())})
joblib.dump(BGmu3,'A5_BGmu3')
...
joblib.dump(dimdict3,'A5_dimdict3')
...
reflection_dict=joblib.load('A5_reflection_dict')
...
#W = joblib.load('A5_Wa_to30')
#Admmu1 = joblib.load('A5_Admmu1')
#Admmu2 = joblib.load('A5_Admmu2')
#Admmu3 = joblib.load('A5_Admmu3')
#BGmu1 = joblib.load('A5_BGmu1')
#BGmu2 = joblib.load('A5_BGmu2')
#BGmu3 = joblib.load('A5_BGmu3')
