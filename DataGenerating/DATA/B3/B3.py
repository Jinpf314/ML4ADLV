...
semisimplerank = 3
alpha = dict()
alpha[1]=rational_vect([1/2,-1/2,0,0,0,1/2,-1/2])
alpha[2]=rational_vect([0,1/2,-1/2,0,1/2,-1/2,0])
alpha[3]=rational_vect([0,0,1/2,0,-1/2,0,0])

alphacheck = dict()
alphacheck[1]=vect([1,-1,0,0,0,1,-1])
alphacheck[2]=vect([0,1,-1,0,1,-1,0])
alphacheck[3]=vect([0,0,2,0,-2,0,0])

#def Kotteq(w,nubar):
#    return(abs( (w.t.t).sum() - (nubar.t).sum())<0.001)
omegacheck=dict()
omegacheck[3]=vect([1,1,1,0,-1,-1,-1])
omegacheck[2]=vect([1,1,0,0,0,-1,-1])
omegacheck[1]=vect([1,0,0,0,0,0,-1])
omega=dict()
omega[3]=rational_vect([1/4,1/4,1/4,0,-1/4,-1/4,-1/4])
omega[2]=rational_vect([1/2,1/2,0,0,0,-1/2,-1/2])
omega[1]=rational_vect([1/2,0,0,0,0,0,-1/2])


e0 = vect( np.zeros(rank) )
thetacheck = vect([1,1,0,0,0,-1,-1])
theta = rational_vect([1/2,1/2,0,0,0,-1/2,-1/2])

s = dict()
s[1] = affweyl(e0, permutation([2,1,3,4,5,7,6]))
s[2] = affweyl(e0, permutation([1,3,2,4,6,5,7]))
s[3] = affweyl(e0, permutation([1,2,5,4,3,6,7]))
s[0] = affweyl(thetacheck, permutation([6,7,3,4,5,1,2]))
...
for i in S0key:
    S0.add(s[i])
S = {s[0]} | S0
Wf = joblib.load('B3_Wf')
Phiplus = joblib.load('B3_Phiplus')
two_rho = joblib.load('B3_two_rho')
rho = joblib.load('B3_rho')
...
'''
Admmu2 = Adm(mu2,tau2)
joblib.dump(Admmu2,'B3_Admmu2')
    
BGmu2 = set()
for i in Admmu2:
    for w in Admmu2[i]:
        BGmu2 = vectunion(BGmu2,{rational_vect(w.Newton())})
joblib.dump(BGmu2,'B3_BGmu2')
vectprint(BGmu2)
dimdict2 = dict()
dimset = dict()
for nu in BGmu2:
...
joblib.dump(dimdict2,'B3_dimdict2')
...
reflection_dict=joblib.load('B3_reflection_dict')
