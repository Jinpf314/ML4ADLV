...
semisimplerank = 4
alpha = dict()
alpha[1]=rational_vect([1/2,-1/2,0,0,0,0,0,1/2,-1/2])
alpha[2]=rational_vect([0,1/2,-1/2,0,0,0,1/2,-1/2,0])
alpha[3]=rational_vect([0,0,1/2,-1/2,0,1/2,-1/2,0,0])
alpha[4]=rational_vect([0,0,0,1/2,0,-1/2,0,0,0])

alphacheck = dict()
alphacheck[1]=vect([1,-1,0,0,0,0,0,1,-1])
alphacheck[2]=vect([0,1,-1,0,0,0,1,-1,0])
alphacheck[3]=vect([0,0,1,-1,0,1,-1,0,0])
alphacheck[4]=vect([0,0,0,2,0,-2,0,0,0])

#def Kotteq(w,nubar):
#    return(abs( (w.t.t).sum() - (nubar.t).sum())<0.001)
omegacheck=dict()
omegacheck[4]=vect([1,1,1,1,0,-1,-1,-1,-1])
omegacheck[3]=vect([1,1,1,0,0,0,-1,-1,-1])
omegacheck[2]=vect([1,1,0,0,0,0,0,-1,-1])
omegacheck[1]=vect([1,0,0,0,0,0,0,0,-1])
omega=dict()

omega[4]=rational_vect([1/2,1/2,1/2,1/2,0,0,0,0,0])
omega[3]=rational_vect([1/2,1/2,1/2,0,0,0,-1/2,-1/2,-1/2])
omega[2]=rational_vect([1/2,1/2,0,0,0,0,0,-1/2,-1/2])
omega[1]=rational_vect([1/2,0,0,0,0,0,0,0,-1/2])


e0 = vect( np.zeros(rank) )
thetacheck = vect([1,1,0,0,0,0,0,-1,-1])
theta = rational_vect([1/2,1/2,0,0,0,0,0,-1/2,-1/2])

s = dict()
s[1] = affweyl(e0, permutation([2,1,3,4,5,6,7,9,8]))
s[2] = affweyl(e0, permutation([1,3,2,4,5,6,8,7,9]))
s[3] = affweyl(e0, permutation([1,2,4,3,5,7,6,8,9]))
s[4] = affweyl(e0, permutation([1,2,3,6,5,4,7,8,9]))
s[0] = affweyl(thetacheck, permutation([8,9,3,4,5,6,7,1,2]))
...
for i in S0key:
    S0.add(s[i])
S = {s[0]} | S0
W0 = joblib.load('B4_W0')
Phiplus = joblib.load('B4_Phiplus')
two_rho = joblib.load('B4_two_rho')
rho = joblib.load('B4_rho')
w_0 = affweyl(e0, permutation(np.arange(1,rank+1)[::-1]) )

mu1 = omegacheck[1]
y01 = exp([2,3,2,4,3,2,4,3,4]) + w_0
...
'''
Admmu2 = Adm(mu2,tau2)
joblib.dump(Admmu2,'B4_Admmu2')
    
BGmu2 = set()
for i in Admmu2:
    for w in Admmu2[i]:
        BGmu2 = vectunion(BGmu2,{rational_vect(w.Newton())})
joblib.dump(BGmu2,'B4_BGmu2')
vectprint(BGmu2)
dimdict2 = dict()
dimset = dict()
for nu in BGmu2:
...
joblib.dump(dimdict2,'B4_dimdict2')
...
reflection_dict=joblib.load('B4_reflection_dict')
