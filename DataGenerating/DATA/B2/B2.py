...
semisimplerank = 2
alpha = dict()
alpha[1]=rational_vect([1/2,-1/2,0,1/2,-1/2])
alpha[2]=rational_vect([0,1/2,0,-1/2,0])


alphacheck = dict()
alphacheck[1]=vect([1,-1,0,1,-1])
alphacheck[2]=vect([0,2,0,-2,0])

#def Kotteq(w,nubar):
#    return(abs( (w.t.t).sum() - (nubar.t).sum())<0.001)
omegacheck=dict()
omegacheck[2]=vect([1,1,0,-1,-1])
omegacheck[1]=vect([1,0,0,0,-1])
omega=dict()
omega[2]=rational_vect([1/4,1/4,0,-1/4,-1/4])
omega[1]=rational_vect([1/2,0,0,0,-1/2])


e0 = vect( np.zeros(rank) )
thetacheck = vect([1,1,0,-1,-1])
theta = rational_vect([1/2,1/2,0,-1/2,-1/2])

s = dict()
s[1] = affweyl(e0, permutation([2,1,3,5,4]))
s[2] = affweyl(e0, permutation([1,4,3,2,5]))
s[0] = affweyl(thetacheck, permutation([4,5,3,1,2]))
...
Deltacheck = set(alphacheck.values()) 
...
for i in S0key:
    S0.add(s[i])
S = {s[0]} | S0
Wf = joblib.load('B2_Wf')
print(f'len(Wf) = {len(Wf)}') 
Phiplus = joblib.load('B2_Phiplus')
two_rho = joblib.load('B2_two_rho')
rho = joblib.load('B2_rho')
...
'''
Admmu2 = Adm(mu2,tau2)
joblib.dump(Admmu2,'B2_Admmu2')
    
BGmu2 = set()
for i in Admmu2:
    for w in Admmu2[i]:
        BGmu2 = vectunion(BGmu2,{rational_vect(w.Newton())})
joblib.dump(BGmu2,'B2_BGmu2')
vectprint(BGmu2)
dimdict2 = dict()
dimset = dict()
for nu in BGmu2:
...
joblib.dump(dimdict2,'B2_dimdict2')
...
reflection_dict=joblib.load('B2_reflection_dict')
