...
alpha = dict()
alpha[1]=rational_vect([1/2, -1/2, 1/2,-1/2])
alpha[2]=rational_vect([0,1,-1,0])
...
omega=dict()
omega[2]=rational_vect([1,1,0,0])
omega[1]=rational_vect([1,0,0,0])
omegacheck=dict()
omegacheck[2] = vect([1,1,0,0])
omegacheck[1] = vect([2,0,0,0])

e0 = vect( np.zeros(rank) )
theta = rational_vect([1,0,0,-1])
thetacheck = vect([1,0,0,-1])
s = dict()
s[1] = affweyl(e0, permutation([2,1,4,3]))
s[2] = affweyl(e0, permutation([1,3,2,4]))
s[0] = affweyl(theta, permutation([4,2,3,1]))
...
for i in S0key:
    S0.add(s[i])
S = {s[0]} | S0
Wf = joblib.load('C2_Wf')

Phiplus = joblib.load('C2_Phiplus')
two_rho = joblib.load('C2_two_rho')
rho = joblib.load('C2_rho')
...
'''
Admmu2 = Adm(mu2,tau2)
joblib.dump(Admmu2,'C2_Admmu2')
    
BGmu2 = set()
for i in Admmu2:
    for w in Admmu2[i]:
        BGmu2 = vectunion(BGmu2,{rational_vect(w.Newton())})
joblib.dump(BGmu2,'C2_BGmu2')
vectprint(BGmu2)
dimdict2 = dict()
dimset = dict()
for nu in BGmu2:
...
joblib.dump(dimdict2,'C2_dimdict2')
...
reflection_dict=joblib.load('C2_reflection_dict')
...
################################
#W = joblib.load('C2_W_to30')
#Admmu2 = joblib.load('C2_Admmu2')
#BGmu2 = joblib.load('C2_BGmu2')
