...
alpha = dict()
alpha[1] = rational_vect([1/2, -1/2, 0, 0, 0, 0, 1/2, -1/2])
alpha[2] = rational_vect([0, 1/2, -1/2, 0, 0, 1/2, -1/2, 0])
alpha[3] = rational_vect([0, 0, 1/2, -1/2, 1/2, -1/2, 0, 0])
alpha[4] = rational_vect([0, 0, 0, 1,-1, 0, 0, 0])

omega=dict()
omega[4] = rational_vect([1/2,1/2,1/2,1/2,-1/2,-1/2,-1/2,-1/2])
omega[3] = rational_vect([1/2,1/2,1/2,0,0,-1/2,-1/2,-1/2])
omega[2] = rational_vect([1/2,1/2,0,0,0,0,-1/2,-1/2])
omega[1] = rational_vect([1/2,0,0,0,0,0,0,-1/2])

omegacheck=dict()
omegacheck[4] = vect([1,1,1,1,0,0,0,0])
omegacheck[3] = vect([1,1,1,0,0,-1,-1,-1])
omegacheck[2] = vect([1,1,0,0,0,0,-1,-1])
omegacheck[1] = vect([1,0,0,0,0,0,0,-1])

e0 = vect( np.zeros(rank) )
theta = rational_vect([1,0,0,0,0,0,0,-1])
thetacheck = vect([1,0,0,0,0,0,0,-1])
...
for i in S0key:
    S0.add(s[i])
S = {s[0]} | S0
W0 = joblib.load('C4_W0')

Phiplus = joblib.load('C4_Phiplus')
two_rho = joblib.load('C4_two_rho')
rho = joblib.load('C4_rho')
...
'''
Admmu4 = Adm(mu4,tau4)
joblib.dump(Admmu4,'C4_Admmu4')
    
BGmu4 = set()
for i in Admmu4:
    for w in Admmu4[i]:
        BGmu4 = vectunion(BGmu4,{rational_vect(w.Newton())})
joblib.dump(BGmu4,'C4_BGmu4')
vectprint(BGmu4)
dimdict4 = dict()
dimset = dict()
for nu in BGmu4:
...
for nu in dimset:
    dimdict4[nu] = max(dimset[nu])
    print(nu.t,dimdict4[nu] )
joblib.dump(dimdict4,'C4_dimdict4')
...
reflection_dict=joblib.load('C4_reflection_dict')
...
##########################
#W = joblib.load('C4_Wa_to30')

#Admmu4 = joblib.load('C4_Admmu4')
#BGmu4 = joblib.load('C4_BGmu4')
