...
parser = argparse.ArgumentParser(description='Generation program for dataset 3. You can run this program several times in parallel to populate the database faster. Make sure to start the different processes with unique random seeds. The Cartan Type used in the original paper is A4.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
db.argparseAddCommonArguments(parser, tablename_default='dataset3')
...
while True:
	...
	while sum(mu_indices) != 0 or len(set(mu_indices)) != rank+1:
		mu_indices = list(sorted(random.randint(-8,8) for _ in range(rank+1)))
	...
	w = functions.We_W0P.from_translation(mu) * functions.We_W0P.from_classical_weyl(y)
	print(w)
